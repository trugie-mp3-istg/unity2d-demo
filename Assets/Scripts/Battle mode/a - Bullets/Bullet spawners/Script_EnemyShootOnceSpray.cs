...
public class Script_EnemyShootOnceSpray : BulletSpawnerBase {

    [Header("Spawner-specifics - Spray")]
    ...
    private void SpawnBullet(float angle) {
        if (bullet) {
            ...
            spawnedBullet.transform.rotation = transform.rotation * Quaternion.Euler(0f, 0f, angle + Random.Range(-randomSpread, randomSpread));
        }
    }
}
